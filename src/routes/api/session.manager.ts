...
export class SessionManager {
    ...
    static addSession = (sessionId: string, session: Session): Promise<Session | null> => {
        if (!session?.sessionId) {
            console.log("session has no session id")
            return null;
        }
    ...
    static isValid = (sessionId: any): Promise<boolean> => {
    ...
    static removeSession = (sessionId: any): Promise<Session | null> => {
        const session = SessionManager._sessions.find((x) => x.sessionId === sessionId);
        if (!session) {
    ...
    static constructSession = async (user: { SessionId: any; UserId: any; Email: any; UserName: any; profileImageUrl: any; fullName: any; firstName: any; CurrentRoleId: any; }, token: string, expiryDate: Date): Promise<Session> => {
